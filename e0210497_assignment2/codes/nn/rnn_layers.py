...
class RNN(Layer):
    ...
    def forward(self, inputs):
        ...
        h = np.random.uniform(size=(T, N, H))
        h[-1] = self.h0
        ...
        for t in range(T):
            if t == 0:
                ...
                h_prev = h[t-1]
            h[t] = RNNCell.forward(self, [inputs[t], h_prev])

        ## [N, T, H]
        output = h.transpose(1, 0, 2)
    ...
    def backward(self, out_grad, inputs):
        ...
        N, T, D = inputs.shape
        H = out_grad.shape[2]
        h = np.nan_to_num(self.forward(inputs))
        in_grad = np.zeros((N, T, D))
        out_grad_prev = out_grad[:, T - 1, :]

        for t in reversed(range(T)):
            if t > 0:
                h_prev = h[:, t - 1, :]
            else:
                h_prev = np.reshape(np.repeat(self.h0, N), (N, H), order=True)
            [in_grad[:, t, :], out_grad_prev], kernel_grad, r_kernel_grad, b_grad = self.cell.backward(
                out_grad_prev, [inputs[:, t, :], h_prev], self.kernel, self.recurrent_kernel, self.bias)
            self.kernel_grad += kernel_grad
            self.r_kernel_grad += r_kernel_grad
            self.b_grad += b_grad

            if t > 0:
                out_grad_prev = out_grad[:, t - 1, :] + out_grad_prev
...
class BidirectionalRNN(Layer):
    ...
    def backward(self, out_grad, inputs):
        ...
        ###############################################
        # TODO: code here
        N, T, H_double = out_grad.shape
        H = H_double // 2
        ## N(batch) * T(time_steps) * D(in_features)

        # forward: (batch, time_steps, in_features)
        forward_input_grad = self.forward_rnn.backward(out_grad[:, :, 0:H], inputs)
        # backward: (batch, time_steps, in_features)
        mask = ~np.any(np.isnan(inputs), axis=2)
        backward_input_grad = self.backward_rnn.backward(self._reverse_temporal_data(out_grad[:, :, H:H_double], mask),
                                                self._reverse_temporal_data(inputs, mask))
        backward_input_grad = self._reverse_temporal_data(backward_input_grad, mask)
        in_grad = sum([forward_input_grad, backward_input_grad])
