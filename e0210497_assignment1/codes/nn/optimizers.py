...
class Adam(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        for k in list(xs.keys()):
            #########################################
            # TODO: code here
            self.accumulators[k] = self.beta_1*self.accumulators[k]+(1-self.beta_1)*xs_grads[k]
            self.moments[k]=self.beta_2*self.moments[k]+(1-self.beta_2)*(xs_grads[k]**2)
            if self.bias_correction:
                new_xs[k] = xs[k]-self.lr*(self.accumulators[k]/(1-self.beta_1**iteration))/ \
                            (np.sqrt(self.moments[k]/(1-self.beta_2**iteration)) + self.epsilon)
            else:
                new_xs[k] = xs[k]-self.lr * self.accumulators[k]/(np.sqrt(self.moments[k])+self.epsilon)
            #########################################
        return new_xs
